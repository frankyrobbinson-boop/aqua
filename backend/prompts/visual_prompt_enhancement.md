...
- **NO instruments, tags, or numbers.** NEVER place a ruler, tape measure, measuring stick, scale, gauge, thermometer, or dial, or a tag, label, sign, or packet, or ANY number or graduation in the image, and NEVER add one the `visual_description` didn't name — AI models render them garbled or blank. When the narration is about a measurement or a named variety, render the ACTION (hands placing the item, the item itself), not an instrument or a labeled tag (reinforces Rule 5).
...
5. **No watermarks, logos, or visible signage.** End every prompt with `no text, no numbers, no rulers, no measuring tools, no labels, no watermarks, no logos`.
