...
One continuous thread, not a montage. Pick the strongest single image from the outline's hook fields and let the other elements ride inside it - the failure they've lived, the blame lifted off them, the claim that justifies the click, the bookmark. State the title's actual answer plainly inside the first ~300 words - early payoff, not a slow build. The promise points at their yard, never at the runtime: no "by the end of this video you'll know." Name no items; if a bookmark is set, tease its importance, never its content. Roll straight into section 1.

# Sections

One per outline item, each written as its turn: open on the belief or the failure - not the secret itself; the reveal is what the section builds to. Land the reveal, prove it with plain cause-and-effect, then give the fix as action - what to do, when, what to touch, with the real numbers. `key_material` is a pantry, not a checklist - use what serves the turn and leave the rest out. When the landing lands, the section ends.
