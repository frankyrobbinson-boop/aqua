...
- **Mark the peak.** Flag the one or two highest-feeling sections "peak" in `purpose`, and at least two as "escalation." The hook's bookmark points at the peak that carries the act — and that bookmarked peak must sit in the SECOND HALF of the list (for a 5-item video, item 4 or 5), never item 1 or 2, so its payoff lands later instead of early.
- **Plan the stitches - backward only.** When a later item builds on an earlier one (fix the form → the lift that finally stops hurting), note it in `purpose` so the script can call back. Never plan a forward reference to an unrevealed item; the bookmark is the video's only forward pointer.
...
- **`section.title` is a short literal label for the mistake/secret** (e.g. "The timing," "The panic fix") - downstream stages key off this string; the intrigue lives in `purpose` and the script.
...
- **hook** subfields are concepts, not prose. `click_justifying_claim` = the most click-worthy true thing you have. `private_failure` = the failure they've lived, named as theirs. `felt_pain_image` = that failure as it looks right now (the peeling paint, the crooked shelf). `external_villain` = the advice/tag/myth that led them into it. `end_state` = the after-state in their space once they act - never phrased "by the end of this video." `tension_into_section_1` = the thread that hands off. `numbered_bookmark` = optional; the phrase teases the payoff's IMPORTANCE ("the one that explains why most of these fail in the first month"), never its content - no rule or answer inside the phrase. Point it at a second-half peak — `section_index` sits in the back half of the list (for a 5-item video, item 4 or 5), never item 1 or 2; set section_index null when unused.
